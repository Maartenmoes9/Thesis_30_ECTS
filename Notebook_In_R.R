...
# Specify the GARCH(1,1) student T model with the exogenous variables
garch_std.exo.spec <- ugarchspec(
  variance.model = list(model = "sGARCH", garchOrder = c(1, 1)),
  mean.model = list(armaOrder = c(1, 0), include.mean = TRUE, external.regressors = as.matrix(df[, c("reg_output_squared", "Multiplication_dummy")])),
